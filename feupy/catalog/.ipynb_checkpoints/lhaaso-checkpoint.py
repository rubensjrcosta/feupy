...
class SourceCatalogObjectExtraLHAASO(SourceCatalogObject):
    ...
    def flux_points(self):
        ...
        file_path = ""
        filename = f"$PYTHONPATH/data/dedicated_publications/lhaaso/2024icrc.confE.643Y/{name_to_fname(self.name)}.fits"
...
# Association Name: {data['ASSO_Name']}
# Association Separation: {data['ASSO_Sep']:.3f}
        
        for which in ['KM2A', 'WCDA']:
            if self.spectral_model(which) is not None:
                model = self.spectral_model(which)
                parameters = model.parameters
                ss += f"Model: {which}\n"
                ss += f"Spectrum type: {model.tag[0]}\n"
                for par in parameters:
...
#     @property
    def flux_points(self, which):
        """Return flux points (`~gammapy.estimators.FluxPoints`)."""
        table = self.flux_points_table(which)
        meta = table.meta
        return FluxPoints.from_table(
            table=table,
            reference_model=self.spectral_model(which),
            sed_type=meta['SED_TYPE'],
        )
    
#     @property
    def flux_points_table(self, which):
        """Flux points table as a `~astropy.table.Table`."""
        
        data = self.data

#         if data['Model_a'] == which:
#             tag = ""
#         elif data['Model_b'] == which: 
#             tag =  "_b"
#         else: raise ValueError("which needs to be 'KM2A' or 'WCDA'")

#         e_ref =  u.Quantity([data[f'E0{tag}']]) 

        e_ref = u.Quantity([self._get("E0", which)]) 

        sed_type = 'dnde'
        spec_model = self.spectral_model(which=which)
        dnde = spec_model(e_ref)
        dnde_err = spec_model.evaluate_error(e_ref)[1]
        is_ul = False
        dnde_ul = np.nan
        ...
        table["is_ul"] = is_ul
        table["is_ul"].description = 'Whether data is an upper limit.'
...
class SourceCatalog1LHAASO(SourceCatalog):
    ...
    def __init__(self, filename="$PYTHONPATH/data/catalogs/1lhaaso/1LHAASO_catalog.ecsv"):
        table = Table.read(make_path(filename), format='ascii.ecsv')
...
# convert_ipynb_to_gallery('Untitled.ipynb', "lhaaso.py")
